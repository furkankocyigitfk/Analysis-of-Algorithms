...
struct node{//film veya oyuncuya ait bilgileri tutar.
	...
	int index;//eklendiði yerin indis bilgisini tutar.
	struct node *next;
};
...
struct stack{//oyuncularda farkli kombinasyonlari tutmak icin stackler ard arda eklenerek tutulmustur.
	int len;//yiginin uzunlugu
	struct node *top;//yiginin en ustunu gosteren pointer
	struct stack *next;//bir sonraki stackin adresi
};
struct queue{//bfs yapýlýrken node larý gezmek icin kullanilmistir.
	struct stack *rear;
	struct stack *front;
};
void push(struct stack *s,char *data,int index){//verilen stackin en ustune verilen bilgilere ait node eklenir.
...
void addEdge(Graph *g,int index,char *artist){//verilen grafadaki verilen film indexi ile verilen oyuncu bilgisi arasinda kenar olusuturur.
...
void bfs(Graph *g,char *first,char *second,int f){//verilen grafta,verilen iki oyuncu arasindaki izlenen yolu f degiskenine gore bulur.
												  //f 1 iken aranacak yoldaki aralýk maximum 6 olur.0 oldugunda ise 100 olur.
	...
	while(q->front != NULL && flag){
		...
		if(cur->len / 2 > count){//maximum aranan derinlikte bulunamadi ise cikar.
			...
			if(strcmp(cur->top->data,second) == 0){//aranan kisi bulunduysa stack yazdirilir ve program biter.
				printf("Yolun uzunlugu: %d ve bulunan yol:\n",cur->len/2);
				print_stack(cur);
				flag=0;
			}
			else{
				iter=g->artist[cur->top->index]->next;//kuyrugun basindaki adamin oynadýgi filmler
				while(iter != NULL){
...
int search(Graph *g,char *str){//graftaki oyuncular hashlenerek eklendigi icin eklenecegi indexi dondurur.
