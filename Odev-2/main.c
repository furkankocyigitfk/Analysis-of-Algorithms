...
int main()
{
	char *filename = "samples.txt";			//source dosyalar�n�n isimlerinin tutuldugu dosya
	char addDocumentFileName[16];			//yeni eklenecek dosyalar icin kullanilmistir
	Node *hashTable = make_table(filename); //source deki dosyalara gore hashtable �n olusturulmas�
	...
	while (x)
	{
		...
		if (option == 1)
		{
			printTable(hashTable); //hashtable � yazd�r�r.
		}
		else if (option == 2)
		{
...
void addDocumentTable(Node *table, char *filename)
{									   //ismi verilen dosyadan key i olusturur table a bakar.Eger key in gosterdigi deger bossa ekler.
									   //Degilse ve dosyalar ayn� ise eklemez.Dosyalar farkl� ise bos yer gorene kadar bu olay devam eder
	lld key = getHashNumber(filename); //Dosyadan keyi elde etme
	if (key == -1)
	{ //Dosya acilamadi ise key -1 dir
	...
	printf("%d\n", index);
	while (flag && table[index].val != NULL)
	{ //dosyalar ayni degilse ve bakilan index bos degilse doner
		//printf("%s %s\n",table[index].val,filename);
		if (compareDocuments(table[index].val, filename) == 1)
		{ //dosyalar ayni ise flag guncellenir ve dongu son bulur.
			flag = 0;
		}
		else
		{ //dosyalar ayni degil ise yeni index elde edilir
			i++;
			printf("%d %d ", index, i);
			index = funcH(key, i);
			printf("%d\n", index);
	...
	if (flag)
	{ //Dosyalar ayni degilse tabloya eklenir.
...
Node *make_table(char *filename)
{											//hash table � okunan dosya ismine gore olusturur.
	...
	if (fp_source == NULL)
	{ //dosya acilamazsa fonksiyon null dondurur
		printf("No Source File");
		return NULL;
	}
	Node *hashTable = (Node *)malloc(sizeof(Node) * TABLE_SIZE); //table � olusturma
	for (i = 0; i < TABLE_SIZE; i++)
	{ //table i sifirlama
		hashTable[i].val = NULL;
	}
	while (!feof(fp_source))
	{ //kaynak dosyas�n� sonuna kadar okuma
...
int search(Node *table, lld key)
{							   //verilen keye karsilik tablodaki bos yerin indexini dondurur
	int i = 0;				   //kacinci seferde bos yere eklendiginin bilgisini tutar
	int index = funcH(key, i); //ilk sefer icin elde edilen index
	printf("%d ", index);
	while (table[index].val != NULL)
	{		 //eger bakilan yer bos degilse
...
{																			 //adi verilen dosyayi dizene kaydeder
...
lld get_key(char *word, int n)
{				 //okunan kelimeye ait key degerini dondurur
...
lld getHashNumber(char *filename)
{				   //verilen dosya ismini okur ve ona ait key degerini uretir
	...
	while (fscanf(fp_assitant, "%c", &ch) != EOF)
	{ //karakter okuma
		if ((ch <= 'Z' && ch >= 'A') || (ch <= 'z' && ch >= 'a'))
		{ //karakter alfabetik ise word e eklenir
			word[j++] = ch;
			word[j] = '\0';
		}
		if ((ch == ' ') || ((ch == '\n') && (bch == '.')))
		{										//karakter bosluk ise veya dosya sonu geldiyse
...
int funcH(lld key, int i)
{ //hash fonksiyonu verilen key in tablodaki indexini dondurur
	return (funcH1(key) + i * funcH2(key)) % TABLE_SIZE;
}
int funcH1(lld key)
{ //hash1 fonksiyonu double hashing icin gerekli ilk fonksiyon
	return key % TABLE_SIZE;
}
int funcH2(lld key)
{ //hash2 fonksiyonu double hashing icin gerekli ikinci fonksiyon
